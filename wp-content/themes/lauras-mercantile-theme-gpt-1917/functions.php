...
/**
 * /shop-only: enforce deterministic category grouping for the React shop grid.
 * Fail-open and scoped: does not run on cart/checkout/product pages.
 */
function lm_enqueue_shop_ordering() {
  // Run only on the /shop page path (React route page).
  ...
  $theme_path = get_stylesheet_directory();
  $theme_uri  = get_stylesheet_directory_uri();
  $js_path = $theme_path . '/assets/js/shop-ordering.js';
  if (file_exists($js_path)) {
    wp_enqueue_script(
      'lm-shop-ordering',
      $theme_uri . '/assets/js/shop-ordering.js',
      array(),
      filemtime($js_path),
      true
    );
...
add_action('wp_enqueue_scripts', 'lm_enqueue_shop_ordering', 96);
...
?>
        <div class="lm-blog-card-body">
          <h2 class="lm-blog-card-title"><a href="<?php the_permalink(); 
...
?></a></h2>
          <p class="lm-blog-card-meta"><?php echo esc_html( get_the_date() ); 
...
?></p>
          <p class="lm-blog-card-excerpt"><?php echo esc_html( wp_trim_words( get_the_excerpt(), 26, '…' ) ); 
...
?>"><?php esc_html_e( 'Keep Reading', 'lauras-mercantile' ); 
...
/* ===== Cart: remove "You may be interested in…" cross-sells =====
 * This disables cross-sells for BOTH classic WooCommerce cart templates and the WooCommerce Blocks cart.
 */
add_action('wp', function () {
  if (is_admin()) return;
...
// Hard-disable cross-sell query counts (classic template)
add_filter('woocommerce_cart_cross_sells_total', function ($total) { return 0; }, 999);

// WooCommerce Blocks cart (Cart block / Store API rendering)
if (!function_exists('__return_false')) {
  function __return_false() { return false; }
}
add_filter('woocommerce_blocks_cart_show_cross_sells', '__return_false', 999);
